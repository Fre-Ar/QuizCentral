...
# Quiz Example with 1 page and 1 MCQ 1 option question
```typescript
// Quiz Schema
{
  "id": uuid;
  ...
  "config": {
    "time_limit_seconds"?: number; // If 0 or undefined, infinite.
    "navigation_mode": "linear" | "free"; // linear cannot go back, free can
  },
  ...
  "blocks": [
    {
      "id": "q1",
      "type": "interaction_unit",
      "domain_id": "booleans", // primitive domain containing true and false
          ...
          "children": [
            ...
            },
            {
              ...
              "props": {
                ...
                "styling": { "classes": [] }, // default styling
            
                "state_logic"?: {
                  "disabled"?: {
                      "==": [ { "var": "value" }, true ] // value refers to the IU's value
                  }
                },
    ...
    },
    {
      "id": "q2",
      "type": "interaction_unit",
      "domain_id": [[1], [2], [3], [4]], // elements are 1-element arrays which can have an integer 1,2,3, or 4.
            ...
            },
            {
              ...
              "props": {
                "behavior": {
                  ...
                  "pick_n": null // Default null (Show all)
                },
                ...
                "children": [
                  {
                    ...
                    "props": {
                      ...
                      "label": "Paris", 
                      "label_position": "end", // "end" means the label is after the variant's icon
...
## Turning the question into a template:
```typescript
{
  "type": "template_instance", // template instance type means it uses a template and fills its parameters
  "id": "q2",

  "template_id": "tpl_mcq_single", 

  "state": {
    // init value of state not needed since it is defined by the template
    // visited is the same init value as the template
    "required"?: false // overriding the template's required: true
  },
    
  "behavior"?: {
    // this behavior overrides or adds to the underlying behavior of the template
    "hidden"?: {
      "==": [ { "var": "q1.value" }, false ]
    },
    ...
    "listeners"?: {
      ...
      // the listerner for quiz.timer is not needed because the template already defines it
    }
  },
  "parameters": {
...
```typescript
{
  "type": "template", // template type means it is an actual template, not an instance
  "id": "tpl_mcq_single", // this is the template_id of the template_instance block
  "name": "Scored MCQ",  // name of the templated, displayed in the UI
  "parameters": {
    "question": "string", // the value of the parameter names is immediately their type written in string, no need for {"type": "string"}. The default value is the "empty" value of the types (0 for number, false for bool, empty string for string). However, the default type of array-type and object-type parameters is an array with one element of its defined type or an object with every one of its properties initialized at their default values.
    "options": [ // if the parameter expects an array, the value is just an array
      { "label": "string", "value": "any", "points": "number" } // if the parameter expects an object, then you just have the object with a name: type definition.
    ],

    "container_style": [ "style_id" ], // style_id is a special type. domain_id also exists. The default value of these is null (here, the array would init as empty)
  ...
  "structure": {
    "type": "interaction_unit",
    "domain_id": {
      "$$map": { // {"$$map": ...} is an expression that takes in a {"source", "as", "template"} object and generates an array it its place
        "source": {"param": "options"}, // Iterate over parameters.options
        "as": "opt",         // Local variable name for the loop
        
        // The Template to clone for each item (can be an object, an array, a primitive, a block, etc)
        "template": [ {"var": "opt.value" } ] // the schema {"var": "opt.value" } is converted to whatever  "opt.value" is (in this case, an integer)
        // using "var" in {"var": "opt"} is only possible inside of a $$map because of "as":"opt".
      }
    }, // this $$map definition creates the array [ [1], [2], [3], [4] ] for an options array of length 4 with values 1,2,3,4
    ...
    "behavior"?: {
      "quiz.timer": { 
        "if": [
          ...
          { "+=": [ 
              { "var": "quiz.score" }, 
              "$$switch": {  // switch is similar to a switch statement in which 
                "on": {"var": "value"}, 
                "cases": { // cases is an array of {"match", "result"} objects. Here, $$map creates this array dynamically depending on .
                  "$$map": {
    ...
    "view": {
      ...
      "props": {
        "behavior": {
          "shuffle_children": {"param": "suffle_options"}, 
          "pick_n": null
        },
                    ...
                    "props": {
                      ...
                      "events"?: {
                        "on_click"?: { 
                          "set": [ 
                            { "var": "value" }, 
                            [ {"var": "opt.value" } ] // [i] where i is 1..N
                          ] 
                        }
                      }
                    }
